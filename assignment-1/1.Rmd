...
compute_splits <- function(x) {
  ...
  (x_sorted[1:length(x_sorted)-1] + x_sorted[2:length(x_sorted)])/2
}
...
tree_grow <- function(x, y, nmin, minleaf, nfeat) {
  ...
  feat <- rownames(splt_imp)[splt_imp[,"impurity"] == min(splt_imp[,"impurity"])][1]
  split <- splt_imp[feat,"split"]
  xl <- x[x[[feat]] <= split,]
  xr <- x[x[[feat]] > split,]
  yl <- y[x[[feat]] <= split]
  yr <- y[x[[feat]] > split]
  node <- c(feat, split, list(row.names(xl)), list(row.names(xr)))
  tree <- c(tree, node)
  while (nrow(xl) <= nmin) {
    splt_imp <- per_feat(use_feats, xr, yr, minleaf)
    splt_imp <- splt_imp[complete.cases(splt_imp),]
    if (nrow(splt_imp) != 0) {
      feat <- rownames(splt_imp)[splt_imp[,"impurity"] == min(splt_imp[,"impurity"])][1]
      split <- splt_imp[feat,"split"]
      xl <- xl[xl[[feat]] <= split,]
